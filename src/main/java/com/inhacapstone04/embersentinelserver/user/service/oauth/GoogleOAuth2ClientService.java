...
import com.inhacapstone04.embersentinelserver.user.entity.oauth.GoogleUserInfo;
...
public class GoogleOAuth2ClientService implements OAuth2ClientService {
    ...
    public OAuth2UserInfo getUserInfo(String accessToken) {
                ...
                .onStatus(HttpStatusCode::isError, response ->
                        response.bodyToMono(String.class)
                                .flatMap(errorBody -> Mono.error(
                                        new CustomException(ErrorCode.INVALID_TOKEN, "Google 토큰 검증 실패: " + errorBody)
                                ))
                )
        ...
        if (attributes == null) {
            throw new CustomException(ErrorCode.INVALID_TOKEN, "Google 사용자 정보를 가져오지 못했습니다.");
        }
